import { DEFAULT_SERVER_URL } from './appConfig.js';
...
export async function exportVideo({
  ...
  await exportFast(sourceVideo, sourceFile, start, end, layout, titleLines, metaLines, titleTop, metaTop);
}
...
export async function exportVideoFast({
  ...
  if (primeVideoOverlayCache && sourceFile) {
...
async function exportVideoOnServer({
  sourceFile,
  start,
  end,
  layout,
  ...
  const serverUrl = getVideoServerUrl();
  const duration = Math.max(0.1, end - start);
  setVideoSaveProgress(3, '서버 렌더링 준비 중');

  try {
    const overlayBlob = await primeVideoOverlayCache();
    const body = new FormData();
    body.append('video', sourceFile, sourceFile.name || 'source.mp4');
    body.append('overlay', overlayBlob, 'overlay.png');
    body.append('start', String(start));
    body.append('duration', String(duration));
    body.append('layout', JSON.stringify({
      frame: {
        x: layout.frame.renderX ?? layout.frame.x,
        y: layout.frame.renderY ?? layout.frame.y,
        width: layout.frame.width,
        height: layout.frame.height
      }
    }));

    setVideoSaveProgress(15, '서버로 전송 중');
    const response = await fetch(`${serverUrl}/api/render-video`, {
      method: 'POST',
      body
    });
    if (!response.ok) {
      throw new Error(`서버 렌더링 실패 (${response.status})`);
    }

    setVideoSaveProgress(92, '영상 받는 중');
    const payload = await response.json();
    const videoUrl = new URL(payload.videoUrl, serverUrl).toString();
    const link = document.createElement('a');
    link.href = videoUrl;
...
function getVideoServerUrl() {
  const stored = window.localStorage?.getItem('videoServerUrl') || '';
